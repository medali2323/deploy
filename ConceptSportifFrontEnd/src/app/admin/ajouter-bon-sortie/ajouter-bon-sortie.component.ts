...
@Component({
  selector: 'app-ajouter-bon-sortie',
  templateUrl: './ajouter-bon-sortie.component.html',
  styleUrls: ['./ajouter-bon-sortie.component.css']
})
export class AjouterBonSortieComponent {
  ...
  total_ttc_be:number=0
    ...
    this._erreur.pipe(debounceTime (3000)).subscribe(() => { if (this.selfClosingAlert) { this.selfClosingAlert.close();
      ...
      this.getall_bonsorties();
   
    }
  
    getall_bonsorties(): void {
  ...
  removeRow(){
    ...
    let nouvelleBonSansIds = { 
      code: 'BS_'+(this.nb+1).toString(),
      date: this.nouvelleBon.datebe,
      fournisseur: this.nouvelleBon.Fournisseurs,
      montantTTC:this.nouvelleBon.total_ttc_be,
      totalHT:0
     
    };
    this.httpservice.create("bonsorties", nouvelleBonSansIds).subscribe((response: any) => {
      const b=response
      console.log(response);
      let t=0;
      ...
      b.totalHT=t
        this.httpservice.update("bonsortiess", response.id,b).subscribe((response2: any) => {
          console.log(response);
        ...
        this._success.next("Le bon de sortie a été ajouté avec succès.");
