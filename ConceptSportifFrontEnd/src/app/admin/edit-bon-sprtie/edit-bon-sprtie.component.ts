...
@Component({
  selector: 'app-edit-bon-sprtie',
  templateUrl: './edit-bon-sprtie.component.html',
  styleUrls: ['./edit-bon-sprtie.component.css']
})
export class EditBonSprtieComponent {
  BonSortie:any
  Fournisseurs: any[] = [];
  ...
  selectedfournisseurId: number | null = null;
    ...
    getBonSortie(id: number): void {
      this.httpservice.getById("bonsorties",id)
        .subscribe((data) => {
          console.log(data);
          ...
          const dateObject = new Date(this.BonSortie.date); // Convert string to Date object
    ...
    getall_BonSortie(): void {
      this.httpservice.getAll("bonsorties").subscribe((response: any) => {
  ...
  fillTable(id: any) {
    ...
    this.httpservice.getById("ligneBonSorties/byBonSortie", id).subscribe((response: any) => {
      console.log(response);
      this.tab1 = response;
  ...
  add_ligne_table_prod() {
    ...
    this.tabupdateqte = this.tab1.filter((e: any) => e.produit.id === a.id && e.BonSortie.id === this.id && e.quantite != a.qte);
  ...
  ajouternouvelleBon(f: NgForm): void {
    ...
    for (const e of this.tab) {
      if (e.idl) {
        ...
        this.httpservice.update("ligneBonSorties", e.idl, aux).subscribe((response1: any) => {
        ...
        this.httpservice.create("ligneBonSorties", aux).subscribe((response1: any) => {
          console.log(response1);
        }, (error: any) => {
          alert('Erreur lors de l\'ajout du categorie_representant : ' + error.message);
        });
      }
      this.httpservice.update("bonsorties", this.id, this.BonSortie).subscribe((response2: any) => {
        console.log(response2);
        this.route.navigate(['/admin/stock-vente/bon_sortie/liste']);
      }, (error: any) => {
        alert('Erreur lors de l\'ajout du categorie_representant : ' + error.message);
      });
    }
